...
func Test_generatePasswordHash(t *testing.T) {
	t.Run("argon2id hash being generated correctly", func(t *testing.T) {
		gotPasswordHash, err := generatePasswordHash([]byte("Sommer2019"), []byte("somegoodsalt"), pkg.PasswordHashArgon2ID)
		assert.NoError(t, err)
...
func Test_generatePasswordUserEntry(t *testing.T) {
	t.Run("salt being generated is of correct length", func(t *testing.T) {
		_, salt, err := generatePasswordUserEntry([]byte("Sommer2019"), pkg.PasswordHashArgon2ID)
		assert.NoError(t, err)
		assert.Len(t, salt, 16)
	})
	t.Run("password hash is being generated correctly", func(t *testing.T) {
		password := []byte("Sommer2019")
		gotPasswordHash, salt, err := generatePasswordUserEntry(password, pkg.PasswordHashArgon2ID)
		assert.NoError(t, err)
		expectedPasswordHash, err := generatePasswordHash(password, salt, pkg.PasswordHashArgon2ID)
		assert.NoError(t, err)
